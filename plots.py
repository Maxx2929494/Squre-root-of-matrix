...
def plot_convergence_six(it_lists, err_lists, labels=None, logy=True, figsize=(7,4.5), linewidth=1.5, markersize=5, filename=None):
...
x = [np.arange(1,51) for _ in range(6)]
y = [np.exp(-0.1*i*np.arange(1,51)) + np.random.rand(50)*1e-3 for i in range(1,7)]


fig, ax = plot_convergence_six(x, y, labels=[f'alg {i}' for i in range(1,7)])
